...
class KNMIDownloader:
    ...
    def _get_valid_temporary_folder(
            self,
            temporary_download_folder: Union[str, None],
            clear_files_from_folder: bool
    ):
        ...
        if not possible_download_folder.exists():
            ...
            try:
                ...
                raise FileNotFoundError(f'The supplied download folder for KNMIDownloader was invalid. '
                ...
                raise FileExistsError(f'Inconsistency error when creating the download folder for KNMIDownloader: '
        ...
        if not possible_download_folder.is_dir():
            raise FileExistsError('The given target folder name for KNMIDownloader already existed, but as a file! '
        ...
        if next(possible_download_folder.iterdir(), None) is not None and not clear_files_from_folder:
            ...
            raise FileExistsError('Files exist within the target directory for KNMIDownloader, but file deletion is '
        ...
        if next(possible_download_folder.iterdir(), None) is not None and clear_files_from_folder:
            ...
            try:
                ...
                self.logger.error('An unexpected Exception occurred while clearing the target directory for '
                                  'KNMIDownloader:')
    ...
    def get_all_available_files(self) -> List[str]:
        ...
        """
        self.logger.info(f'Fetching the full list of files available for dataset [{self.dataset_name}], '
    ...
    def download_specific_file(self, filename: str, filesize: int):
        ...
        if self.temporary_download_folder.joinpath(filename).exists() and \
                self.temporary_download_folder.joinpath(filename).stat().st_size == filesize:
    ...
    def _download_and_save_file_by_name_and_size(self, temporary_download_url: str, filesize: int):
        ...
        if total_bytes_transferred != filesize:
            raise EOFError(f'The downloaded file for [{file_name_of_download}] did not match the expected file size:'
                           f' {filesize} bytes')
