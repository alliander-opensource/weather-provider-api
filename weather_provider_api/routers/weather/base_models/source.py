...
class WeatherSourceBase(object):  # pragma: no cover
    ...
    def __init__(self, source_id: str, name: str, url: str, model_instances: list[WeatherModelBase], *args: Any, **kwargs: Any) -> None:
        """Initialize the WeatherSourceBase with an ID and set up the models."""
        self.id = source_id
        self.name = name
        self.url = url
        self._models: dict[str, WeatherModelBase] = {model.id: model for model in model_instances if not model.async_model}
        self._async_models: dict[str, WeatherModelBase] = {model.id: model for model in model_instances if model.async_model}
