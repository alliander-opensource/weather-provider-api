...
from weather_provider_api.app.routers.weather.api_models import OutputUnit
from weather_provider_api.app.routers.weather.sources.knmi.models.actuele_waarnemingen import ActueleWaarnemingenModel
from weather_provider_api.app.routers.weather.sources.knmi.models.daggegevens import DagGegevensModel
from weather_provider_api.app.routers.weather.sources.knmi.models.pluim import PluimModel
from weather_provider_api.app.routers.weather.sources.knmi.models.uurgegevens import UurgegevensModel
from weather_provider_api.app.routers.weather.utils.geo_position import GeoPosition
from weather_provider_api.app.routers.weather.utils.pandas_helpers import coords_to_pd_index
...
def test_convert_names_and_units(mock_single_value_dataset):
    ...
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.si)['temperature'][0][0] == 298.15
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.human)['temperature'][0][0] == 25.0
    ...
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.si)['precipitation'][0][0] == 0.032
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.human)['precipitation'][0][0] == 32
    ...
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.original)['mock_unknown_field'][0][
            0] == 66
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.si)['mock_unknown_field'][0][0] == 66
    assert \
        base_model.convert_names_and_units(mock_single_value_dataset, OutputUnit.human)['mock_unknown_field'][0][
            0] == 66
...
# TODO: When the new Harmonie Arome Model is finished, add it to the tests.
def test__request_weather_factors():
    ...
    assert base_model._request_weather_factors(weather_factors_input) == ['wind_speed', 'wind_direction',
                                                                          'short_time_wind_speed', 'temperature',
                                                                          'precipitation', 'precipitation_sum', 'cape']
    ...
    assert sorted(base_model._request_weather_factors(weather_factors_input)) == ['precipitation',
    ...
    weather_factors_input = ['WIND_SPEED', 'duck_feathers', 'precipitation', 'temperature']

    # Pluim - full return of factors,
    assert sorted(base_model._request_weather_factors(weather_factors_input)) == ['precipitation', 'temperature',
                                                                                  'wind_speed']

    # Daggegevens - return only known factors
    extra_model = DagGegevensModel()
    weather_factors_input = ['FHNH', 'fxx', 'duck_feathers', 'T10NH']
    assert sorted(extra_model._request_weather_factors(weather_factors_input)) == ['FHNH', 'FXX', 'T10NH']

    # Uurgegevens - return only known factors
    extra_model = UurgegevensModel()
    weather_factors_input = ['DDVEC', 'T10N', 'duck_feathers', 'vv']
    assert sorted(extra_model._request_weather_factors(weather_factors_input)) == ['DDVEC', 'T10N', 'VV']

    # ActueleWaarnemingen - return only known factors
    extra_model = ActueleWaarnemingenModel()
    weather_factors_input = ['weather_description', 'visibility', 'duck_feathers', 'AIR_pressure']
    assert sorted(extra_model._request_weather_factors(weather_factors_input)) == ['air_pressure', 'visibility',
                                                                                   'weather_description']

    # TEST 4: Only unknown factors
    # Unknown factors should be removed
...
@pytest.mark.parametrize("wind_direction,resulting_degrees", [
    ("N", 360.0),
    ("NNO", 22.5),
    ("NO", 45.0),
    ("ONO", 67.5),
