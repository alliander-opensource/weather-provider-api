...
from weather_provider_api.app.routers.weather.api_models import ScientificJSONResponse
...
def test_custom_json_float_encoder():
    ...
    mock_response = ScientificJSONResponse({'coords': {
        'time': {'dims': ('time',), 'attrs': {'long_name': 'time'},
                 'data': [datetime.utcnow()],
                 'coord': {'dims': ('coord',), 'attrs': {}, 'data': [(5.25, 52.0)]}}, 'attrs': {},
        'dims': {'coord': 1, 'time': 744}, 'data_vars': {
            '2m_dewpoint_temperature': {'dims': ('time', 'coord'),
                                        'attrs': {'units': 'K', 'long_name': '2 metre dewpoint temperature'},
