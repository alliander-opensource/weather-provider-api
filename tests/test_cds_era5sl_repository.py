...
def _get_mock_prefix(dummy_date: datetime):
    ...
    return (
        era5sl_repo.file_prefix
        + "_"
        + str(dummy_date.replace(day=1).year)
        + "_"
...
def _fill_mock_repository(_get_mock_repository_dir: Path):
    ...
    mock_file_dates = [
        ...
        datetime.utcnow() + relativedelta(months=-4),
    ]
...
def test_repository_should_file_update(_get_mock_repository_dir: Path):
    ...
    mock_file = era5sl_repo.repository_folder.joinpath(
        _get_mock_prefix(datetime.utcnow() + relativedelta(months=-1))
    )
    ...
    os.rename(
        mock_file.with_suffix(".nc"),
        Path(str(mock_file) + "_INCOMPLETE").with_suffix(".nc"),
    )
    ...
    # Expected result:  The response should indicate that an update is required
    new_file_prefix = _get_mock_prefix(
        datetime.utcnow()
...
def test_repo_get_month_filename(_get_mock_repository_dir: Path):
    ...
    assert (
        len(glob.glob(str(era5sl_repo.repository_folder.joinpath("ERA5SL")) + "*.*"))
        == 10
    ...
    file_prefix = era5sl_repo.repository_folder.joinpath(
...
def test_finalize_formatted_file(_get_mock_repository_dir):
    ...
    if era5sl_repo.repository_folder.exists():
        shutil.rmtree(era5sl_repo.repository_folder)
    era5sl_repo.cleanup()
    mock_file_prefix = str(
        era5sl_repo.repository_folder.joinpath(era5sl_repo.file_prefix)
    )
    ...
    file_date = datetime.utcnow() - relativedelta(
        months=(era5sl_repo.age_of_permanence_in_months + 2)
    )
    mock_file = (
        mock_file_prefix
        + "_"
    ...
    era5sl_repo.cleanup()
    file_date = datetime.utcnow() - relativedelta(
        months=(era5sl_repo.age_of_permanence_in_months - 1)
    )
    mock_file = (
        mock_file_prefix
        + "_"
...
def test_era5sl_repository_update(
    monkeypatch, _get_mock_repository_dir, mock_coordinates
):
    ...
    if era5sl_repo.repository_folder.exists():
        shutil.rmtree(era5sl_repo.repository_folder)
    era5sl_repo.cleanup()
    months_in_full_update = (
        (
            era5sl_repo.get_last_day_of_repo().year
            - era5sl_repo.get_first_day_of_repo().year
        )
        * 12
        + (
            era5sl_repo.get_last_day_of_repo().month
            - era5sl_repo.get_first_day_of_repo().month
        )
        + 1
    )

    # MOCKING: Intercepting the ERA5SL Download function and returning a dummy dataset for further handling
    def mock_download_era5sl_file(
        ...
        ds = xr.Dataset(
            data_vars=data_dict,
            coords={"time": timeline, "longitude": [coord[1]], "latitude": [coord[0]]},
        )
        ds.to_netcdf(path=target_location, format="NETCDF4", engine="netcdf4")

    monkeypatch.setattr(
        ERA5SLRepository, "_download_era5sl_file", mock_download_era5sl_file
    )
    ...
    files_in_repository = glob.glob(
        str(era5sl_repo.repository_folder.joinpath("*.*"))
    ...
    Path(files_in_repository[random_number]).rename(
        str(file_to_rename[:-3]) + "_UNFORMATTED.nc"
    )
    Path(files_in_repository[second_random_number]).rename(
        str(second_file_to_rename[:-3]) + "_FORMATTED.nc"
    )
